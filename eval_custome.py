...
def Run_video(dataset,video, num_frames,start_frame, num_objects,model,Mem_every=None, Mem_number=None):
    ...
    for t in range(1,num_frames-1):
        ...
        if not os.path.exists(test_path.replace("results","logits/forward")):
            os.makedirs(test_path.replace("results","logits/forward"))
        torch.save(E,os.path.join(test_path.replace("results","logits/forward"), jpg_filename+'.pt'))
        del logit
        # update
