...
for iter_ in range(args.total_iter):
	...
	if (iter_+1) % save_step == 0 and (iter_+1) >= args.total_iter * 0.8:
		...
		torch.save(model.state_dict(), os.path.join(args.save,'davis_{}_{}_{}_{}_{}.pth'.format(args.name,args.backbone,str(args.total_iter),str(args.batch),str(iter_+200000))))
