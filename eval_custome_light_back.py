...
### My libs
from dataset.dataset_back import DAVIS_MO_Test
...
def Run_video(dataset,video, num_frames,start_frame, num_objects,model,Mem_every=None, Mem_number=None):
    ...
    test_path = os.path.join('/mnt/storage/home/ru20956/scratch/results_b', video)
    if not os.path.exists(test_path):
        os.makedirs(test_path)
    palette = Image.open('/mnt/storage/home/ru20956/scratch/DAVIS/Annotations/480p/bear/00000.png').getpalette()

    #jpg_filename = dataset.load_single_image_name(video, 0)
    #jpg_filename_path = dataset.load_single_image_name_path(video, 0)
    #shutil.copyfile(jpg_filename_path, os.path.join(test_path, jpg_filename+'.png'))

    #jpg_filename = dataset.load_single_image_name(video, num_frames-1)
    #jpg_filename_path = dataset.load_single_image_name_path(video,num_frames-1)
    #shutil.copyfile(jpg_filename_path, os.path.join(test_path, jpg_filename+'.png'))
    ...
    for t in range(1,num_frames-1):
        ...
        if not os.path.exists(test_path.replace("results_b","logits/backward")):
            os.makedirs(test_path.replace("results_b","logits/backward"))
        log, prd  = torch.max(E[0],dim=0)
        #prd = prd.cpu().numpy().astype(np.uint8)
        l = torch.stack([log, prd])
        torch.save(l,os.path.join(test_path.replace("results_b","logits/backward"), jpg_filename+'.pt'))
