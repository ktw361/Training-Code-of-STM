...
class DAVIS_MO_Test(data.Dataset):
    ...
    def __init__(self, root, imset='2017/train.txt', resolution='480p', single_object=False,direction_m=False):
        self.root = root
        self.mask_dir = '/mnt/storage/scratch/ru20956/logits/results'
        self.mask480_dir = '/mnt/storage/scratch/ru20956/logits/results'
    ...
    def __getitem__(self, index):
        ...
        if not isinstance(index, int):
            ...
            _mask = np.array(Image.open(sorted(glob.glob(os.path.join(self.mask_dir, video, '*.png')),reverse=self.direction_m)[self.start_frame_for_tracking_len[video] - 1]).convert("P"))
    ...
    def load_single_image(self,video,f):
        ...
        try:
            mask_file = sorted(glob.glob(os.path.join(self.mask_dir, video,"*.png")),reverse=self.direction_m)[f+self.start_frame_for_tracking_len[video]-1] # -1 since last and fist frames are not part of the png data
